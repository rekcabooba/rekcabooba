...
<table class="table table-bordered table-hover table-striped"  style="background-color:rgba(237, 231, 225, 0.5);margin-top:50px" align="center" >
		...
		$sql="select*from customer_registration,login where login.uname=customer_registration.email ";
		...
		while($row=$result->fetch_assoc())
		{
            ...
            echo"<td>".$row['phno']."</td>";
             if($row['status']=="0")
            {
                echo"<td><a href='updateuser1.php?id=".$row['email']."&status=1'>Accept</td>";
                echo"<td><a href='updateuser1.php?id=".$row['email']."&status=-1'>Reject</td>";
            }
             if($row['status']=="-1")
            {
               echo"<td>Rejected</td>"; 

            ...
            <nav class="navbar navbar-expand-lg navbar-light stroke py-lg-0">
                ...
                <div class="collapse navbar-collapse" id="navbarScroll">
                    <ul class="navbar-nav ms-auto my-2 my-lg-0 navbar-nav-scroll">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="bookdisp.php">booked slot</a>
                        </li>
...
<table class="table table-bordered table-hover table-striped"  style="background-color:rgba(237, 231, 225, 0.5);margin-top:50px" align="center" >
		...
		$sql="select*from collection_agent,login where login.uname=collection_agent.caemail ";
		$result=$conn->query($sql);
		while($row=$result->fetch_assoc())
		{
			echo"<tr><td>".$row['caid']."</td>";
			echo"<td>".$row['caname']."</td>";
			echo"<td>".$row['address']."</td>";
            echo"<td>".$row['cacontact']."</td>";
            echo"<td>".$row['caemail']."</td>";
            echo"<td>".$row['caaadhar']."</td>";
            echo"<td>".$row['area']."</td>";
            if($row['status']=="0")
            {
                echo"<td><a href='updateuser.php?id=".$row['caemail']."&status=1'>Accept</td>";
                echo"<td><a href='updateuser.php?id=".$row['caemail']."&status=-1'>Reject</td>";
            }
            if($row['status']=="-1")
            {
               echo"<td>Rejected</td>"; 
